...
import 'package:flutter_file/services/backend_launcher.dart'; // ← start backend exe
import 'package:open_filex/open_filex.dart';

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  // Nyalakan backend (akan extract exe dari assets & tunggu /ping OK)
  try {
    await BackendLauncher.start();
  } catch (e) {
    debugPrint('Gagal start backend: $e'); // biar ketahuan di console
  }
  runApp(const MyApp());
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  @override
  void dispose() {
    // Matikan backend ketika app ditutup
  ...
  Future<void> proses() async {
    ...
    try {
      ...
      // NOTE: kalau tak mau langsung terbuka (agar tidak ke-lock),
      // komentari baris berikut.
      await OpenFilex.open(result.path);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('File disimpan: ${result.path}')),
        );
      }
    } catch (e) {
      if (mounted) {
      ...
      body: Container(
        ...
        decoration: const BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [
              Color.fromARGB(255, 228, 105, 132),
              Color.fromARGB(255, 240, 170, 185),
            ],
          ),
        ),
